...
// Generate a set of sample users
...
// Generate a set of sample todos
export const generateSampleTodos = (count: number = 200): Todo[] => {
  ...
  return Array.from({ length: count }).map((_, index) => {
    ...
    return {
      id: `todo-${index + 1}`,
      title: `Task ${index + 1}: ${['Complete', 'Review', 'Plan', 'Discuss', 'Implement'][Math.floor(Math.random() * 5)]} ${['project', 'report', 'meeting', 'feature', 'bug fix'][Math.floor(Math.random() * 5)]}`,
      completed: Math.random() > 0.7,
      description: `This is a detailed description for task ${index + 1}. It contains a lot of unnecessary text to make the payload larger. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.`,
